...
classdef planeControler < handle
    % Class to control the plane, using the flight plans and the plane's state
    properties
        AltitudeControler;
        RouteControler;
        RudderControler;
    ...
    methods
        function obj = planeControler(...
                altitude_controler_params, route_controler_params, ...
            ...
            obj.RouteControler = routeControler(route_controler_params);
        ...
        function [u, intermediary_terms] = step(obj, x, turb, route_target, target_airspeed, target_altitude, flaps_instruction)
            ...
            %   airspeed
            %   - target_altitude in m
            %   - flap_instruction in rad

            % Returns :
            %   - u : the command to apply to the plane
            %   - intermediary_terms : the intermediary terms used to compute the command
            % The descriptions of the different controlers can be found in the control diagram files
            ...
            phir = x(10);
            [target_gamma, u_elevator] = obj.AltitudeControler.step(...
                delta_h, h_d, gamma, gamma_d, alpha, tas);
            
            route = atan2(plane_speed_E(2), plane_speed_E(1));
            route_error = route_target - route;
            route_error = wrapToPi(route_error);
            rr = x(9);
            pr = x(7);
            [u_aileron, target_roll_angle] = obj.RouteControler.step(...
                route_error,rr,phir,pr, tas);
