function [x_hist, u_hist, intermediary_terms_hist, usefullvar_list] = ...
    ...
    % Function to test the route angle control parameters using a Heaviside
    % function. The plane starts with 1s at a target route of 0 then 119s with a target route of target_route
    % Inputs:
    %   route_pid_params: 1x3 array with the PID parameters for the route angle controler
    ...
    roll_angle_pid_kp = route_pid_params(1);
    roll_angle_pid_ki = route_pid_params(2);
    roll_angle_pid_kd = route_pid_params(3);
    max_roll_angle = 30*pi/180;
    aileron_pid_kp = 3296.1;
    aileron_pid_ki = 0;
    aileron_pid_kd = 4172;
    ...
    rudder_controler_params=[0.2, 0.01, 0.1 ,delta_t, 30*pi/180];
