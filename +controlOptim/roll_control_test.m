function [x_hist, u_hist, intermediary_terms_hist, usefullvar_list] = ...
    ...
    initial_altitude, initial_mass, target_roll_angle, flap_instruc, use_turb, plotplotplot)
    % Function to test the roll control parameters using a Heaviside
    % function. The plane starts with 1s at a target roll angle of 0 then 109s with a target roll angle
    % of target_roll_angle
    % Inputs:
    %   roll_pid_params: 1x3 array with the PID parameters for the roll controler
    ...
    %   target_roll_angle: target roll angle of the plane in rad
    ...
    import control.planeControlerRollTest
    ...
    roll_kp = roll_pid_params(1);
    roll_ki = roll_pid_params(2);
    roll_kd = roll_pid_params(3);
    max_aileron_angle = 20*pi/180;
    roll_controler_params=[roll_kp, roll_ki, roll_kd, delta_t, ...
        max_aileron_angle];
    ...
    controler = planeControlerRollTest(altitude_controler_params, ...
        roll_controler_params, rudder_controler_params, ...
    ...
    instruction_hist = [
        ...
        t_change, target_roll_angle, initial_airspeed, initial_altitude, flap_instruc
        t_end, target_roll_angle, initial_airspeed, initial_altitude, flap_instruc
    ];
