function [x_hist, u_hist, intermediary_terms_hist, usefullvar_list] = ...
    ...
    initial_altitude, initial_mass, target_gamma, flap_instruc, use_turb, plotplotplot)
% Function to test the flight path control parameters using a Heaviside
% function. The plane starts with 1s at a target flight path of 0, then 119s with a target flight path of target_gamma
% Inputs:
%   flight_path_pid_params: 1x3 array with the PID parameters for the flight path controler
...
%   target_gamma: target flight path of the plane in rad
...
import control.planeControlerFlightPathTest
...
initial_gamma = 0;
elevator_kp = flight_path_pid_params(1);
elevator_ki = flight_path_pid_params(2);
elevator_kd = flight_path_pid_params(3);
min_elevator = -15*pi/180;
max_elevator = -min_elevator;
initial_elevator = u_init(1);
flight_path_controler_params = [elevator_kp, elevator_ki, elevator_kd, delta_t, initial_elevator, initial_gamma, min_elevator, max_elevator];

controler = planeControlerFlightPathTest(flight_path_controler_params, ...
...
instruction_hist = [
    0, 0, initial_airspeed, 0, flap_instruc
    t_change-0.001, 0, initial_airspeed, 0, flap_instruc
    t_change, 0, initial_airspeed, target_gamma, flap_instruc
    t_end, 0, initial_airspeed, target_gamma, flap_instruc
];
